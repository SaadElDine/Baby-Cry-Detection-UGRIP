...
# Function to merge and combine intervals considering a threshold
def merge_intervals(intervals1, intervals2, intervals3, threshold=0.025):
    ...
    while all_intervals:
        ...
        if merged_intervals and merged_intervals[-1][1] != avg_start:
            avg_start = merged_intervals[-1][1]  # Ensure the end of the last interval is the start of the new one

        if avg_start < avg_end:  # Ensure the start time is smaller than the end time
            merged_intervals.append((avg_start, avg_end))
...
def align_and_merge_annotations(team1, team2, team3, threshold=0.025):
    ...
    for item1, item2, item3 in zip(team1, team2, team3):
        ...
        merged_intervals = merge_intervals(intervals1, intervals2, intervals3, threshold)
        combined_labels = []
...
# Save the combined annotations to a new JSON file for taking average
with open('combined_annotations_Approach1.json', 'w') as f:
    json.dump(combined_annotations, f, indent=4)

print("Done for approach ONE!")